...
template SimpleMultiplier() {
    ...
    // Create constraint here saying that our two input signals cannot equal each other.
    component isz = IsZero();
    isz.in <== in[0] - in[1];

    // The IsZero companent returns 1 if the input is 0, or 0 otherwise.
    isz.out === 0;


    // Define greate than and less than components that we'll define inside the for loo below
    component gte[2];
    component lte[2];
